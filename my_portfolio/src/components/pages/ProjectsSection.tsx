...
import { SiTypescript, SiTailwindcss, SiMongodb, SiNextdotjs, SiFirebase, SiExpress, SiDjango, SiFastapi, SiStripe, SiSqlite, } from "react-icons/si";
...
interface ProjectResponse {
    ...
    image: string;
    link: string;
    logos: string[]; // This matches the backend response format
}
...
interface Project {
    ...
    image: string;
    link: string;
    technologies: string[]; // For internal component use
}

// Expanded tech icon mapping with additional technologies from your backend
const techIcons: { [key: string]: JSX.Element } = {
    ...
    // Additional technologies from your backend
    ...
    "stripe": <SiStripe className="text-purple-500" />,
    "GenAI": <FaPython className="text-blue-300" />,
    "genai": <FaPython className="text-blue-300" />,
    
    "Map Tracking": <FaJs className="text-yellow-300" />,
    "map tracking": <FaJs className="text-yellow-300" />,
...
const ProjectsSection = () => {
    ...
    useEffect(() => {
        const fetchProjects = async () => {
            try {
                ...
                const transformedProjects = data.slice(0, 3).map((item: ProjectResponse) => ({
                    id: item._id,
                    title: item.title,
                    description: item.description,
                    image: item.image,
                    link: item.link,
                    technologies: item.logos || [] // Map logos array to technologies
                }));
    ...
    const getTechIcon = (tech: string) => {
        ...
        return <span className="text-white font-bold">{tech.charAt(0)}</span>;
    };
